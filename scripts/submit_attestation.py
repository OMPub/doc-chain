#!/usr/bin/env python3
"""Submit a signed Doc Chain attestation with Foundry `cast`."""
...
def main() -> int:
    try:
        args = _parse_args()
        signed = load_json(Path(args.signed))
        prepared = signed["prepared"]
        contract_address = normalize_address(
            args.address
            or os.environ.get("DOCCHAIN_ADDRESS")
            or str(prepared["contractAddress"])
        )
        calldata = attest_doc_calldata(prepared["attestation"], str(signed["signature"]))
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Submit a signed Doc Chain attestation.")
    parser.add_argument("signed", nargs="?", default="build/attestations/attestation.signed.json")
